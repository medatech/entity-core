...
async function findEntity({ context, props, type }: { context: Context; props: Record<string, string | number | null | boolean>; type: string }): Promise<EntityType | null> {
    ...
    Object.keys(props).forEach(field => {
        ...
        if (value === null) {
            ...
            query = query.append(`AND props->>'${field}' = `).append(sql`${value} `)
        }
    })
